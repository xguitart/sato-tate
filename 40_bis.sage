...
for s in G:
    if s(rD) == rD and s(r) != r: # sigma should fix the square root of D and it does not fix Q(j)
        sigma = s
        break
# Now we define E, that we have found by a simple search method
a4 = 135*r - 1125
a6 = 6480*r - 5400

# this is E
E = EllipticCurve([0,0,0,H(a4),H(a6)]);
...
##############################################
# Some extra checkings
#############################################
Q.<x> = PolynomialRing(H)
r5 = (x^2 - 5).roots()[0][0]
r40 = (x^2 +40).roots()[0][0]
# H has basis 1, g, g^2, g^3
# Change of matrix from the basis [1, g, g^2, g^3] to [1, r5, r40, r5*r40]
B = Matrix(4,4,[H(1).list(), r5.list(), r40.list(), (r5*r40).list()]).transpose()

x = QQ['x'].gen()
k.<r> = NumberField(x^2 + 40)
Q.<x> = PolynomialRing(k)
F.<s> = NumberField(x^2 - 5)
...
for EC in [E,E_sigma]:
    a4 = EC.ainvs()[3]
    a6 = EC.ainvs()[4]
    aa4 = B.inverse()*a4.vector().column()
    aa6 = B.inverse()*a6.vector().column()
    aa4 = aa4[0][0] + aa4[1][0]*s
    aa6 = aa6[0][0] + aa6[1][0]*s
    Es.append(EllipticCurve([0,0,0,aa4,aa6]))
E_F, E_F_sigma = Es
b = B.inverse()*psi[0].vector().column()
psi_F = x + b[0][0] + b[1][0]*s
